...
struct ComputeResult
{
	...
	ComputeResult() : userPositionInstant(0), readSensorsInstant(0), instant(0), isSuccess(false), errorMessage(L"No result yet") {}
	...
	ComputeResult(uint64_t _userPositionInstant, uint64_t _readSensorsInstant, uint64_t _instant, bool _isSuccess, std::wstring _errorMessage, std::vector<Mote> _motes, GeoPoint _userPosition, Mote _nearestActiveMote) : userPositionInstant(_userPositionInstant), readSensorsInstant(_readSensorsInstant), instant(_instant), isSuccess(_isSuccess), errorMessage(_errorMessage), motes(_motes), userPosition(_userPosition), nearestActiveMote(_nearestActiveMote) {}
};
